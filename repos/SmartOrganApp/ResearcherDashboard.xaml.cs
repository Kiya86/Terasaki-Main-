...
public sealed partial class ResearcherDashboard : Page
{
            ...
            else if (type == 2)
            {
                ...
                if (_isCollecting)
                {
                    ...
                    DispatcherQueue.TryEnqueue(async () =>
                    {
                        ...
                        await RefreshCh1SummaryBarGraphAsync();
                    });
    ...
    private sealed class Ch1BarSummary
    {
        public DateTime Timestamp { get; init; }
        public double AverageLastTenCurrentUa { get; init; }
    }

    private async Task RefreshCh1SummaryBarGraphAsync()
    {
        var summaries = await LoadCh1BarSummariesAsync();

        DispatcherQueue.TryEnqueue(() =>
        {
            WinUIPlot2.Plot.Clear();

            if (summaries.Count == 0)
            {
                WinUIPlot2.Plot.Title("CH1 summary (last 10 avg)");
                WinUIPlot2.Plot.YLabel("Current (uA)");
                WinUIPlot2.Refresh();
                return;
            }

            double[] positions = Enumerable.Range(0, summaries.Count).Select(i => (double)i).ToArray();
            double[] values = summaries.Select(x => x.AverageLastTenCurrentUa).ToArray();
            string[] labels = summaries
                .Select(x => $"ch1\n{x.Timestamp:MM-dd HH:mm}")
                .ToArray();

            WinUIPlot2.Width = Math.Max(600, summaries.Count * 90);
            WinUIPlot2.Plot.Add.Bars(positions, values);
            WinUIPlot2.Plot.Axes.Bottom.SetTicks(positions, labels);
            WinUIPlot2.Plot.Axes.Margins(bottom: 0.2);
            WinUIPlot2.Plot.Title("CH1 summary (last 10 avg)");
            WinUIPlot2.Plot.YLabel("Current (uA)");
            WinUIPlot2.Plot.XLabel("Measurement order (old → new)");
            WinUIPlot2.Plot.Axes.AutoScale();
            WinUIPlot2.Refresh();
        });
    }

    private async Task<List<Ch1BarSummary>> LoadCh1BarSummariesAsync()
    {
        var result = new List<Ch1BarSummary>();
        ...
        foreach (var file in files)
        {
            ...
            if (!string.Equals(parts[^1], "ch1", StringComparison.OrdinalIgnoreCase))
                continue;

            if (!string.Equals(parts[^2], _currentLiverId, StringComparison.OrdinalIgnoreCase))
                continue;

            var stamp = $"{parts[2]}_{parts[3]}";
            if (!DateTime.TryParseExact(stamp, "yyyyMMdd_HHmmss", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime ts))
                continue;
            ...
            result.Add(new Ch1BarSummary
            {
                Timestamp = ts,
                AverageLastTenCurrentUa = lastTen.Average(x => x.Current_uA)
            });
        }
