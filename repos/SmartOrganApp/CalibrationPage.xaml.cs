...
public sealed partial class CalibrationPage : Page
{
    ...
    // ── Google Drive service (resolved from DI, null-safe) ────────────────
    private IGoogleDriveService? _driveService;

    // ── ADC → nA constants (must match firmware) ──────────────────────────
    private const double VrefMv  = 3300.0;
    ...
    protected override async void OnNavigatedTo(NavigationEventArgs e)
    {
        ...
        // Resolve Drive service from DI — null-safe, upload is best-effort
        try
        {
            _driveService = ((App)Application.Current).Host?.Services
                .GetService<SmartOrganApp.Services.Drive.IGoogleDriveService>();
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"[CAL] Drive service unavailable: {ex.Message}");
        }
        ...
        CancellationToken token)
    {
        ...
        for (int chIdx = 0; chIdx < Channels.Length; chIdx++)
        {
            ...
            // Clear live buffers for this channel
            lock (_liveT) { _liveT.Clear(); _liveI.Clear(); }

            SetStatus(statusTb,
                $"{stepName} — CH{ch} {ChNames[chIdx]} ({chIdx + 1}/6) — recording…");

            // 60-second timer: starts immediately, updates every 1s
            // X-axis on graph starts at 0 for each channel
            ...
            // Extract I_final — average of last 10 samples (last 1 second)
            var chRaw = _raw[stepIdx][chIdx];
            double? iFin = null;
            if (chRaw.Count >= 10)
                iFin = chRaw.TakeLast(10).Average(s => s.iNa);
            else if (chRaw.Count > 0)
                iFin = chRaw.Average(s => s.iNa);
            ...
            // 1-second gap between channels
            if (chIdx < Channels.Length - 1)
            {
                SetStatus(statusTb, $"{stepName} — gap before next channel…");
                DispatcherQueue.TryEnqueue(() => timerTb.Text = "0s / 60s");
                await SafeDelay(1100, token);
            }
        }
        ...
        await SaveRawCsvAsync(stepIdx, stepName);
    ...
    $"new _chProg={_chProg}, step={_activeStepIdx}");
        ...
        for (int off = 0; off + pkt <= bytes.Length; off += pkt)
        {
            ...
            // Convert time_ms to seconds for the graph x-axis
            // The PIC resets time_ms to 0 at the start of each channel,
            // so the graph always starts at 0s per channel
            double tSec = tMs / 1000.0;
            ...
            // Write directly to raw store for the active channel
    ...
    private double AdcTomicroA(ushort code)
    {
        ...
        double iUa = (voutMv - VzeroMv) / 1000.0 / RtiaOhm * 1e6;
        return iUa;
    }
    ...
    // ── 1. Raw CSV — time_s, current_nA — one file per channel per step ──
    private async Task SaveRawCsvAsync(int stepIdx, string stepName)
    {
        ...
        for (int chIdx = 0; chIdx < Channels.Length; chIdx++)
        {
            ...
            sb.AppendLine($"# I_final (avg last 10 samples): {_iFinal[stepIdx][chIdx]:F4} nA");
            sb.AppendLine("time_s,current_nA");
